...
parser.add_argument('--video_path', '-i', type=str, help='Video input path')
...
parser.add_argument('--deid', '-m', type=str, default="pixelate", help='De-identification method')
parser.add_argument('--num_iters', type=int, default=30, help='Number of iterations to attack')
...
def inference(images, det_model, align_model):
    ...
    landmarks = [lm.numpy() if mask == 1 else np.zeros((68,2)) for lm, mask in zip(landmarks, masks)]
    ...
    with tqdm(total=NUM_FRAMES) as pbar:
        while CAP.isOpened():
            ...
            if (len(batch)) % BATCH_SIZE == 0 or frame_id==NUM_FRAMES-1:
                ...
                if len(face_boxes) != 0 :
                    for adv_img, face_box, landmark in zip(adv_results, face_boxes, landmarks):
                        image = adv_img.copy()
